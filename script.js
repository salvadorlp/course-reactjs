...
function enter() {
  ...
  if (text == "" || text == null) {
    ...
    return enter();
  } else area.innerHTML = "Welcome " + text;
}
...
function nameButton() {
  ...
  if (buttonName == "" || buttonName == null) {
    ...
    return nameButton();
  } else button.innerHTML = buttonName;
}
...
// Webstorage
// Cookies: data saved in the user browser
//     # Last time that user acess the website
//     # Last page/session acessed by user in the website
//
// Sessions: data temporarily saved in the user browser
//     # data will be lost when the browser is closed
//
// Ways to save with WebStorage
// ----------------------------------------------------------------
// localStorage: data still saved when browser is closed (saved locally in browser)
// sessionStorage: data will be lost when the browser is closed
//
...
function getName() {
  ...
  area.innerHTML = "Storage sending your name: " + name;
}
...
// will pass by each element in array
const mapArray = larray.map((item) => {
  return item * 10;
});

// will pass by each element in array and show it's index
...
// back-end
// # server (PHP, Ruby, NodeJS, Python), databases
//
// front-end
// # visual interface
...
//    - SPA's != (MVC - views)
//       + main page
//       + all of the other pages will be written and modified by JavaScript
//       + instead of returning the formatted HTML, it returns data in json format. -> API's
//       * Back-end and Front-end are independents
//
